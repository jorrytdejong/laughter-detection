...
import os, sys, pickle, time, json, librosa, argparse, torch, numpy as np, pandas as pd, scipy
...
# num_workers=0 avoids multiprocessing spawn issues on macOS when run as script
...
if output_dir:
    ...
    segments = [{"index": i, "start": float(s), "end": float(e), "filename": f"laugh_{i}.wav"} for i, (s, e) in enumerate(instances)]
    with open(os.path.join(output_dir, "laughter_timestamps.json"), "w") as f:
        json.dump({"source_audio": os.path.basename(audio_path), "segments": segments, "total_count": len(segments)}, f, indent=2)
    print("Saved laughter_timestamps.json")
